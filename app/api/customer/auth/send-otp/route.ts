...
export async function POST(req: NextRequest) {
  try {
    ...
    await db.otpCode.create({ data: { email: normalizedEmail, code, expiresAt } });
...
<div style="max-width:480px;margin:0 auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
  ...
  <div style="padding:32px;text-align:center">
    ...
    <div style="background:#F5F0EB;border-radius:12px;padding:24px;margin-bottom:24px;display:inline-block">
      <div style="font-size:42px;font-weight:900;color:#1B4F72;letter-spacing:12px">${code}</div>
    </div>
    <p style="color:#9ca3af;font-size:13px;margin:0">
      ${isEs ? "Válido por 15 minutos." : "Valid for 15 minutes."}
    </p>
  </div>
